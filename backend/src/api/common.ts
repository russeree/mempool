...
export class Common {
  ...
  static isTruc(flags: bigint, tx: TransactionExtended): boolean {
    ...
    // Becuase mempool policy is such that V3 can only have other 3 ancestors / descendants in the mempool
    // The only check we need to do is if there are such TXs currently in the mempool.
    ...
    // If this TX ever achieves a descenant this should be recalculated
    return false;
  }
